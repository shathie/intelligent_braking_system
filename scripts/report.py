...
class ReportGenerator:
    ...
    def _get_avg_accuracy(self) -> float:
        ...
        if 'evaluation_vit' in self.results:
            acc = self.results['evaluation_vit'].get(
                'classification_accuracy',
                self.results['evaluation_vit'].get('accuracy', 0)
            )
            return acc * 100
        return 0.0
    ...
    def _get_executive_summary(self) -> str:
        ...
        if 'evaluation_pinn' in self.results:
            mse = self.results['evaluation_pinn'].get(
                'regression_mse',
                self.results['evaluation_pinn'].get('mse', 1.0)
            )
    ...
    def _get_evaluation_section(self) -> str:
        ...
        if 'evaluation_vit' in self.results:
            ...
            vit_acc = vit_metrics.get('classification_accuracy', vit_metrics.get('accuracy', 0))
            html += """
            <h3>Vision Transformer (ViT)</h3>
            <div class="metrics-grid">
                <div class="metric-card">
        ...
        if 'evaluation_pinn' in self.results:
            ...
            pinn_mse = pinn_metrics.get('regression_mse', pinn_metrics.get('mse', 0))
            pinn_rmse = pinn_metrics.get('regression_rmse', pinn_metrics.get('rmse', 0))
            pinn_mae = pinn_metrics.get('regression_mae', pinn_metrics.get('mae', 0))
            pinn_r2 = pinn_metrics.get('regression_r2', pinn_metrics.get('r2', 0))
            html += """
            <h3>Physics-Informed Neural Network (PINN)</h3>
            <div class="metrics-grid">
                <div class="metric-card">
